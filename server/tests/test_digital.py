...
def test_digital_render_length_bounds_and_determinism():
    ...
    cond = _conditions()
    a = render_reception(Reception.DIGITAL, voice, cond, sample_rate=SR, rng=np.random.default_rng(3))
    b = render_reception(Reception.DIGITAL, voice, cond, sample_rate=SR, rng=np.random.default_rng(3))
...
def test_digital_voice_is_narrowband():
    ...
    out = render_reception(
        Reception.DIGITAL, tone_5k, _conditions(), sample_rate=SR, rng=np.random.default_rng(2)
    )
...
def test_digital_is_cleaner_than_analog_between_words():
    ...
    dig = render_reception(Reception.DIGITAL, voice, cond, sample_rate=SR, rng=np.random.default_rng(1))
    clr = render_reception(Reception.CLEAR, voice, cond, sample_rate=SR, rng=np.random.default_rng(1))
...
def test_digital_holds_clean_copy_on_a_noisy_channel():
    ...
    dig = render_reception(Reception.DIGITAL, voice, noisy, sample_rate=SR, rng=np.random.default_rng(1))
...
def test_digital_falls_off_a_cliff_not_a_slope():
    ...
    good = render_reception(
        Reception.DIGITAL, voice, replace(base, snr_db=8.0), sample_rate=SR, rng=np.random.default_rng(1)
    )
    below = render_reception(
        Reception.DIGITAL, voice, replace(base, snr_db=-4.0), sample_rate=SR, rng=np.random.default_rng(1)
    )
...
def test_digital_failure_includes_garble_not_noise():
    ...
    below = render_reception(Reception.DIGITAL, voice, cond, sample_rate=SR, rng=np.random.default_rng(5))
    analog = render_reception(Reception.CLEAR, voice, cond, sample_rate=SR, rng=np.random.default_rng(5))
...
def test_jamming_defeats_digital_voice_too():
    ...
    jammed = render_reception(
        Reception.DIGITAL, voice, _conditions(jammed=True), sample_rate=SR, rng=np.random.default_rng(1)
    )
    good = render_reception(
        Reception.DIGITAL, voice, _conditions(), sample_rate=SR, rng=np.random.default_rng(1)
    )
