...
def test_digital_render_length_bounds_and_determinism():
    ...
    cond = _conditions()
    a = render_reception(
        Reception.DIGITAL, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(3))
    )
    b = render_reception(
        Reception.DIGITAL, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(3))
    )
...
def test_digital_voice_is_narrowband():
    ...
    out = render_reception(
        ...
        tone_5k,
        _conditions(),
        RenderOptions(sample_rate=SR, rng=np.random.default_rng(2)),
    )
...
def test_digital_is_cleaner_than_analog_between_words():
    ...
    dig = render_reception(
        Reception.DIGITAL, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(1))
    )
    clr = render_reception(
        Reception.CLEAR, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(1))
    )
...
def test_digital_holds_clean_copy_on_a_noisy_channel():
    ...
    dig = render_reception(
        Reception.DIGITAL, voice, noisy, RenderOptions(sample_rate=SR, rng=np.random.default_rng(1))
    )
...
def test_digital_failure_includes_garble_not_noise():
    ...
    below = render_reception(
        Reception.DIGITAL, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(5))
    )
    analog = render_reception(
        Reception.CLEAR, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(5))
    )
