...
def test_clear_render_preserves_length_and_bounds():
    voice = speech_like()
    profile = BandProfile()
    out = render_reception(Reception.CLEAR, voice, profile.conditions_at(145e6), RenderOptions(sample_rate=SR))
...
def test_render_is_deterministic_with_seed():
    ...
    a = render_reception(Reception.CLEAR, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(42)))
    b = render_reception(Reception.CLEAR, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(42)))
...
def test_uhf_cleaner_than_low_hf():
    ...
    uhf = render_reception(
        Reception.CLEAR, voice, profile.conditions_at(440e6), RenderOptions(sample_rate=SR, rng=np.random.default_rng(7))
    )
    low_hf = render_reception(
        Reception.CLEAR, voice, profile.conditions_at(2e6), RenderOptions(sample_rate=SR, rng=np.random.default_rng(7))
    )
...
def test_hash_render_less_intelligible_than_clear():
    ...
    clear = render_reception(Reception.CLEAR, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(5)))
    hashed = render_reception(Reception.HASH, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(5)))
...
def test_with_transients_extends_buffer():
    ...
    plain = render_reception(Reception.CLEAR, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(0)))
    wrapped = render_reception(
        Reception.CLEAR, voice, cond, RenderOptions(sample_rate=SR, rng=np.random.default_rng(0), with_transients=True))
