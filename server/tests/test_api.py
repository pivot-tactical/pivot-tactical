...
def test_instructor_websocket_controls_radio(client):
    ...
    with client.websocket_connect("/ws", cookies={"pivot_token": token}) as wsconn:
        welcome = wsconn.receive_json()
...
def test_instructor_audio_frames_are_tagged_with_radio_id(client):
    ...
    with client.websocket_connect("/ws", cookies={"pivot_token": token}) as instr:
...
def test_event_audio_422_on_render_failure(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "pivot.api.rest.render_event_wav_bytes",
        MagicMock(side_effect=Exception("mocked render error")),
    )
