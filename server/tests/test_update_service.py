...
def test_note_staged_updates_cached_snapshot_and_broadcasts(tmp_path):
    ...
    seen: list[dict] = []
    svc = _service(
        tmp_path, config={"github_repo": "o/r"}, on_change=lambda s: seen.append(dict(s))
    )
...
def test_default_apply_reports_download_progress(tmp_path):
    ...
    with (
        mock.patch.object(mgrmod, "_http_download", fake_download),
        mock.patch.object(mgrmod.UpdateManager, "stage", lambda self, path, rel: self.versions_dir),
    ):
        svc._default_apply(
            Release(tag="1.1.0", asset_url="http://x/a.zip", asset_name="a.zip"), cfg={}
        )
